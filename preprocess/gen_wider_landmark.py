...
def rebuild_file():
    ...
    save_file = '/mnt/data/changshuang/data/test_face_landmark.txt'
...
def main(args):
    ...
    if not os.path.exists(OUTPUT):
        os.mkdir(OUTPUT)
    # 图片处理后输出路径
    dstdir = os.path.join(OUTPUT, 'train_%s_wider_landmark_aug' % (net))
    if not os.path.exists(dstdir):
        os.mkdir(dstdir)
    # label记录txt
    ftxt = os.path.join(data_dir, 'test_face_landmark.txt')
    # 记录label的txt
    f = open(os.path.join(OUTPUT, 'wider_landmark_%d_aug.txt' % (size)), 'w')
    # 获取图像路径, box, 关键点
...
def getDataFromTxt(txt, data_path, with_landmark=True):
    ...
    for line in lines:
        ...
        # 获取图像路径
        img_path = os.path.join(data_path, 'WIDER_train/images/' + components[0]).replace('\\', '/')
        # 人脸box
