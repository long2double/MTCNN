...
def main(args):
    ...
    size = args.input_size  # 12, 24
    batch_size = config.batches  # 384
    min_face_size = config.min_face  # 20
    stride = config.stride  # 2
    thresh = config.thresh  # [0.6, 0.7, 0.7]
    # 模型地址
    ...
    # 图片数据地址
    base_dir = '/mnt/data/changshuang/data/WIDER_train/'
    # 处理后的图片存放地址
    data_dir = '/mnt/data/changshuang/data/%d' % (save_size)
    ...
    mtcnn_detector = MtcnnDetector(detectors, min_face_size=min_face_size, stride=stride, threshold=thresh)

    filename = '/mnt/data/changshuang/data/wider_face_train_bbx_gt.txt'
    # 读取Winder Face文件的image和box对应函数在utils中, note that image is raw image size
...
def save_hard_example(save_size, data, neg_dir, pos_dir, part_dir, save_path):
    ...
    # save files
    neg_label_file = "/mnt/data/changshuang/data/%d/neg_%d.txt" % (save_size, save_size)
    neg_file = open(neg_label_file, 'w')
    pos_label_file = "/mnt/data/changshuang/data/%d/pos_%d.txt" % (save_size, save_size)
    pos_file = open(pos_label_file, 'w')
    part_label_file = "/mnt/data/changshuang/data/%d/part_%d.txt" % (save_size, save_size)
