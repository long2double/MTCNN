...
def train(net_factory, model_prefix, end_epoch, base_dir, display, base_lr):
    ...
    if size == 12:
        ...
        net = 'ONet'
        radio_cls_loss, radio_bbox_loss, radio_landmark_loss = 1.0, 0.5, 1.5

    log = setup_logger('../checkpoint/%s/logs' % net)
    if net == 'PNet':
        # 计算一共多少组数据
        ...
        label_file4=os.path.join(base_dir, 'train_wider_lfw_landmark_aug.txt')
    ...
    sess.run(init)
    if FLAGS.restore:
        model_file = tf.train.latest_checkpoint('../checkpoint/ONet')
...
def random_flip_images(image_batch, label_batch, landmark_batch):
    ...
    if random.choice([0, 1]) > 0:
        ...
        for i in flipindexes:
            cv2.flip(image_batch[i], 1, image_batch[i])  # 1: horizontal flip  0: vertical -1: both axes.
