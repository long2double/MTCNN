...
def main(args):
    ...
    base_dir = os.path.join('/mnt/data/changshuang/data/', str(size))

    if size == 12:
        net = 'PNet'
        net_factory = net_factorys[0]
        end_epoch = FLAGS.end_epoch[0]  # 30
    elif size == 24:
        net = 'RNet'
        net_factory = net_factorys[1]
        end_epoch = FLAGS.end_epoch[1]  # 22
    elif size == 48:
        net = 'ONet'
        net_factory = net_factorys[2]
        end_epoch = FLAGS.end_epoch[2]  # 22
    model_path = os.path.join('../checkpoint/', net)
    if not os.path.exists(model_path):
        os.mkdir(model_path)
    model_prefix = os.path.join(model_path, net)
