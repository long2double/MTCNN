...
if config.input_mode == '1':
    ...
    for item in os.listdir(path):
        ...
        img = cv2.imread("picture/2007_000346.jpg")
    ...
    while True:
            ...
            if ret:
                ...
                for i in range(boxes_c.shape[0]):
                    ...
                    cv2.putText(frame, 'score:{:.2f}  w:{:}   h:{}'.format(score, int(w), int(h)),  (corpbbox[0], corpbbox[1] - 2),  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                    # 画fps值
                cv2.putText(frame, 't:{:.4f}(s)'.format(t) + " " + 'fps:{:.3f}'.format(fps), (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
                # 画关键点
                for i in range(landmarks.shape[0]):
                    for j in range(len(landmarks[i])//2):
